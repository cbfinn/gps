    ...
    def __init__(self, config, quit_on_end=False):
        ...
        if self.using_ioc():
            ...
            if not config['common'].get('nn_demo', False):
                ...
                demo_file = self._hyperparams['common']['experiment_dir'] + 'data_files/' + 'demos_nn_MaxEnt_9cond.pkl'
...
def main():
    ...
    if args.targetsetup:
        ...
        hyperparams.config['common']['data_files_dir'] = exp_dir + 'data_files_maxent_9cond_0/'
        if not os.path.exists(exp_dir + 'data_files_maxent_9cond_0/'):
            os.makedirs(exp_dir + 'data_files_maxent_9cond_0/')
        # hyperparams.config['agent']['randomly_sample_bodypos'] = True
        hyperparams.config['algorithm']['policy_opt']['weights_file_prefix'] = hyperparams.config['common']['data_files_dir'] + 'policy'
        gps = GPSMain(hyperparams.config)
        if hyperparams.config['gui_on']:
