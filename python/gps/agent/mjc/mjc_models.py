...
def half_cheetah_hop_fix_grav(wall_height=0.2, wall_pos=1.2, wall_sample_range=None, mult=10):
    """
     The state space is populated with joints in the order that they are
    defined in this file. The actuators also operate on joints.
    ...
    Actuators (name/actuator/parameter):
        ...
        - ffoot     hinge       torque (N m)
    """
    mjcmodel = MJCModelRegen('half_cheetah', regen_fn=None)
    root = mjcmodel.root
    ...
    default.geom(conaffinity="0", condim="3", contype="1", friction=".4 .1 .1", rgba="0.8 0.6 .4 1",
                 solimp="0.0 0.8 0.01", solref="0.02 1")
    default.motor(ctrllimited="true", ctrlrange=np.array([-1, 1])*mult)
    ...
    actuator = root.actuator()
    gear = 1.0
    actuator.motor(gear=120*gear, joint="bthigh", name="bthigh")
    actuator.motor(gear=90*gear, joint="bshin", name="bshin")
    actuator.motor(gear=60*gear, joint="bfoot", name="bfoot")
    actuator.motor(gear=120*gear, joint="fthigh", name="fthigh")
    actuator.motor(gear=60*gear, joint="fshin", name="fshin")
    actuator.motor(gear=30*gear, joint="ffoot", name="ffoot")
    return mjcmodel
