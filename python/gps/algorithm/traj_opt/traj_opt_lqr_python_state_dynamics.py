...
class TrajOptLQRPythonStateDynamics(TrajOpt):
    ...
    def update(self, m, algorithm):
        ...
        # Estimate dynamics
        _, _ = self.forward(prev_traj_distr, traj_info)
        ...
        for itr in range(DGD_MAX_ITER):
            ...
            traj_distr, eta = self.backward(prev_traj_distr, traj_info,
                                                eta, algorithm, m, traj_info.Fm, traj_info.fv)
    ...
    def forward(self, traj_distr, traj_info):
        ...
        for t in range(T):
            ...
            if t==0:
                Fm_t, fv_t, dyn_covar_t = traj_info.dynamics.linearize(dX, dU, mu[t], mu[t])
            else:
                Fm_t, fv_t, dyn_covar_t = traj_info.dynamics.linearize(dX, dU, mu[t-1], mu[t])

            Fm.append(Fm_t)
            fv.append(fv_t)
            dyn_covar.append(dyn_covar_t)
            if t < T - 1:
                sigma[t+1, idx_x, idx_x] = \
                        Fm_t.dot(sigma[t, :, :]).dot(Fm_t.T) + \
                        dyn_covar_t
                mu[t+1, idx_x] = Fm_t.dot(mu[t, :]) + fv_t
        Fm = np.r_[Fm]
        fv = np.r_[fv]
        dyn_covar = np.r_[dyn_covar]
        traj_info.Fm = Fm
        traj_info.fv = fv
        traj_info.dyn_covar = dyn_covar
        return mu, sigma

    def backward(self, prev_traj_distr, traj_info, eta, algorithm, m, Fm, fv):
        """
